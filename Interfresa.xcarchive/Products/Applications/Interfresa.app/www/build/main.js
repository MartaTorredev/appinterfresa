...
var WysiwygPage = (function () {
    ...
    WysiwygPage.prototype.link = function () {
        var popover = this.popoverCtrl.create(__WEBPACK_IMPORTED_MODULE_11__link_link__["a" /* LinkPage */]);
    ...
    WysiwygPage.prototype.insertVideo = function () {
        this.keyboard.onKeyboardHide().subscribe(function (event) {
            console.log("Cerror teclado");
            console.log(event);
        });
        var popover = this.popoverCtrl.create(__WEBPACK_IMPORTED_MODULE_12__video_video__["a" /* VideoPage */]);
        var selectedElement = this._window.getSelection();
        var first = this._window.getSelection().anchorOffset;
        var range;
        // alert(typeof this._window.getSelection().getRangeAt(0))
    ...
    WysiwygPage.prototype.fontSizePopover = function () {
        var popover = this.popoverCtrl.create(__WEBPACK_IMPORTED_MODULE_6__fontsize_fontsize__["a" /* FontsizePage */]);
        var selectedElement = this._window.getSelection();
        var ev = {
            target: {
        ...
        popover.onDidDismiss(function (data) {
            if (selectedElement.focusNode != null) {
                ...
                selectedElementEdit['style'].fontSize = data + 'px';
            }
        });
    };
    WysiwygPage.prototype.fontFamilyPopover = function () {
        var _this = this;
        var popover = this.popoverCtrl.create(__WEBPACK_IMPORTED_MODULE_7__font_family_font_family__["a" /* FontFamilyPage */]);
        var selectedElement = this._window.getSelection();
        var ev = {
            target: {
        ...
        popover.onDidDismiss(function (data) {
            if (_this._window.getSelection().focusNode != null) {
                ...
                document.execCommand('formatblock', false, formatblock);
                var selectedElementEdit = selectedElement.focusNode.parentNode;
                selectedElementEdit['style'].fontFamily = data;
            }
        });
    };
    WysiwygPage.prototype.paragraphFormatPopover = function () {
        var popover = this.popoverCtrl.create(__WEBPACK_IMPORTED_MODULE_8__paragraph_format_paragraph_format__["a" /* ParagraphFormatPage */]);
        var selectedElement = this._window.getSelection();
        var ev = {
            target: {
    ...
    WysiwygPage.prototype.getImage = function () {
        ...
        // }
        var modal = this.popoverCtrl.create(__WEBPACK_IMPORTED_MODULE_10__rezise_image_rezise_image__["a" /* ReziseImagePage */]);
        var selectedElement = this._window.getSelection();
        var first = this._window.getSelection().anchorOffset;
        var range = undefined;
        if (this._window.getSelection().getRangeAt(0)) {
